...
def reviewer_artifact_files() -> list[Path]:
    suffixes = {".py", ".csv", ".md", ".txt", ".cff", ".pdf", ".zip", ".tex", ".bib"}
    roots = [PACKAGE_ROOT / name for name in ("audit", "docs", "paper")]
    root_names = {
        "README.md",
        "LICENSE",
        "CITATION.cff",
    ...
    files = [
        ...
        if path.is_file() and path.name in root_names
    ]
    for root in roots:
        if not root.exists():
            continue
        files.extend(
            ...
            and path.name != OUT_HASHES
            and ".venv" not in path.parts
            and ".git" not in path.parts
        )
    return sorted(set(files), key=lambda p: package_relative(p).lower())


def check_file_hashes() -> None:
    with (BASE_DIR / OUT_HASHES).open("w", encoding="utf-8", newline="\n") as handle:
        for path in reviewer_artifact_files():
            handle.write(f"{sha256_file(path)}  {package_relative(path)}\n")
...
def write_csv_report(rows: list[dict[str, Any]]) -> None:
    ...
    with (BASE_DIR / OUT_CSV).open("w", encoding="utf-8", newline="") as handle:
...
def final_pass(rows: list[dict[str, Any]], warnings: list[str], args: argparse.Namespace) -> bool:
    ...
    return critical_pass and optional_ok and (BASE_DIR / OUT_HASHES).exists()
...
def write_md_report(rows: list[dict[str, Any]], warnings: list[str], args: argparse.Namespace) -> None:
    ...
    (BASE_DIR / OUT_MD).write_text("\n".join(lines), encoding="utf-8")


def update_manifest() -> None:
    path = BASE_DIR / OUT_MANIFEST
    now = datetime.now().isoformat(timespec="seconds")
    rows = []
    for p in reviewer_artifact_files():
        rows.append(
            {
                "filename": p.name,
                "path": package_relative(p),
